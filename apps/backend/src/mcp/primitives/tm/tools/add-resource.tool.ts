...
export class AddResourceTool {
  private readonly logger = new Logger(AddResourceTool.name);

  constructor(private readonly tmClient: TMClientService) {}

  @Tool({
    name: 'tm_add_resource',
    description:
      'Add a Resource entity to the TM diagram. Resources represent things that are managed or tracked in the domain (e.g. Customer, Product, Order).',
    parameters: z.object({
      name: z.string().describe('Name of the resource entity'),
      subtype: z
  ...
  async addResource(input: any, context: Context) {
    try {
      ...
      this.logger.log(`Adding resource entity: ${input.name} (${entityId})`);
      ...
      return {
        success: true,
        message: `Resource "${input.name}" added successfully with ${attributes.length} attribute(s)`,
        entityId,
        attributeIds,
      };
    } catch (error) {
      this.logger.error('Failed to add resource', error);
      throw error;
    }
  }
}
